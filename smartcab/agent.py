...
class LearningAgent(Agent):
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            if self.epsilon > np.random.rand() and self.epsilon > 0.01:
                action = self.valid_actions[np.random.randint(0, 4)]
            else:
                maxQ = self.get_maxQ(state)
                valid = []
                for temp_act, temp_q in self.Q[state].iteritems():
                    if temp_q == maxQ:
                        valid.append(temp_act)
...
def run():
    ...
    # sim = Simulator(env, update_delay=0.01, log_metrics=True)                           # For default-learning
    sim = Simulator(env, update_delay=0.01, log_metrics=True, optimized=True)             # For optimized-learning
